    ...
    >
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          ...
          <el-button type="danger" size="small" @click="handleDelete(row.username)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  <el-dialog v-model="dialogVisible" :title="dialogMode === 'create' ? '新增用户' : '编辑用户'" width="520px">
    <el-form :model="form" label-width="90px">
      ...
      <el-form-item label="角色" required>
        <el-select v-model="form.roleCode" style="width: 100%" filterable placeholder="请选择角色">
          <el-option
...
const filteredData = computed(() => {
  ...
  return raw.filter(r => {
    ...
    if (!k) return true
    const s = [r?.username, r?.phone, r?.email].filter(Boolean).join(' ').toLowerCase()
    return s.includes(k)
  })
})
...
const getDeptOptions = async () => {
  try {
    deptOptions.value = [{deptNo: null, deptName: '无'}, ...await request.get('/api/dept/')]
  } catch (e) {
    console.error(e)
...
const getRoleOptions = async () => {
  try {
    roleOptions.value = await request.get('/api/sys/role/')
  } catch (e) {
    console.error(e)
    ElMessage.error('获取部门列表失败，' + (e.response?.data?.message || e))
  }
...
const getList = async () => {
  loading.value = true
  try {
    tableData.value = await request.get('/api/sys/user/') || []
    await getDeptOptions()
    await getRoleOptions()
    tableData.value.forEach(row => {
      row.deptName = deptOptions.value.filter(dept => dept.deptNo === row.deptNo)[0]?.deptName || ''
      row.roleName = roleOptions.value.filter(role => role.roleCode === row.roleCode)[0].roleName
    })
  } catch (e) {
    console.error(e)
    ElMessage.error('获取用户列表失败，' + (e.response?.data?.message || e))
  } finally {
    loading.value = false
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    if (dialogMode.value === 'create') {
      await request.post('/api/sys/user/', form.value)
    ...
    console.error(e)
    ElMessage.error('更新失败，' + (e.response?.data?.message || e))
  } finally {
    saving.value = false
  }
}
...
const handleDelete = async (username) => {
  try {
    await ElMessageBox.confirm(`确定删除用户${username}吗？∑( 口 ||`, '提示', {type: 'warning'})
    await request.delete(`/api/sys/user/${username}`)
